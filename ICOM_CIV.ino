...
/*What I've learned...
    ...
    If you send a command that does have return data, you need to WAIT for the data to be deposited in the buffer from the CIV Comms.
       This may take up to 5 seconds or sometimes more.  I often have to resend the request.  (Bluetooth comms seems to be very slow!)

  I need to find a way to send a command, do other processing, and keep returning to the processCatMessages() looking for the response, instead of waiting in the processCatMessages() loop.

   Still trying to figure out how to keep up with the "Transceive" mode frequency change.  When I detect this, I want to stop decoding
      them until I get the last one.  I did run the CIVSniffer.exe (USB) and the USB version is able to keep up with the dial as it
      moves, but the Bluetooth seems to quit after about 4 or 5 seconds.
        THAT DIDN'T WORK.  Tried to stop decoding, but it added too much (slower) code.

  BLUETOOTH module needs to be at 57600,0,0.  Need to use the AT commands to program it, but NEED to use an Arduino MEGA.  NOT a NANO with SofwareSerial.h (not fast enough).
    This is a bit tricky, but if you have setup() with Serial3.begin(57600); and it work, then you have it working.
...
const byte CMD_POWER_OFF = 0x18; //Read operating mode data
const byte TransceiveCmd = 0x1A; //Read the Transceive State
const byte AckMsg = 0xFB; //Received an ACK from the ICOM rig.
const byte NacMsg = 0xFA; //Receive a "Not Good" message from the rig.
const byte CMD_READ_RF_Power = 0x14; //Read operating RF Power value
...
uint8_t bcd2dec(uint8_t n) { // takes 1384 usec for 1000 calls
...
//Commands:
byte ICOM_Rig_Model(void) {
  //There doesn't appear to be a "Rig Model" Other than the RADIO_ADDRESS, to ID the rig.
  //  If we can read the Frequency, then it's my IC-705.
  uint32_t ReadFreq = CIV_Read_Frequency();
  if (ReadFreq != 0) return 3;  // 3 identifies the Arduino MEGA Serial Port 3, that's all...
  else return 0;
}
...
uint32_t CIV_Read_RF_Power() {
  ...
  byte Count = 0;
  //Serial.print(F(" Read."));
  uint8_t RfPower[] = {START_BYTE, START_BYTE, RADIO_ADDRESS, CONTROLLER_ADDRESS, CMD_READ_RF_Power, 0x0A, STOP_BYTE};
